...
import com.zbiti.myapplication.ui.activity.ImageActivity;
...
/**
 * 两处需要注意：
 * 1、#107 --> 说明该GroupImageView必须由LinearLayout包裹
 * 2、#96  --> 说明调用该view前需要给 GlobalParams.screenWidth 赋值
 */
...
//    private PhotoViewAttacher mAttacher;
    ...
    private void displayGroupImageView() {
        ...
        //4个特殊情况，上2个下2个
        if (size == 4) {
            layoutParams = new LinearLayout.LayoutParams(mWidth, imgH * 2 + gap);  // 注意这句话：GroupImageView必须用LinearLayout包裹
            picRects = new Rect[4];
            ...
            //当只有一个图片的时候，特殊处理
            if (size == 1) {
                Rect oneRect = new Rect(0, 0, oneWidth, height);
